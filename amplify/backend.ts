...
backend.auth.resources.unauthenticatedUserIamRole.addToPrincipalPolicy(
  new PolicyStatement({
    effect: Effect.ALLOW,
    actions: ["geo:BatchUpdateDevicePosition", "geo:GetDevicePosition"],
    resources: [
      `arn:aws:geo:${geoStack.region}:${geoStack.account}:tracker/CAParkTracker`,
    ],
  }),
);
...
const geofenceRule = new Rule(geoStack, "GeofenceRule", {
  eventPattern: {
    ...
    resources: [
      `arn:aws:geo:${geoStack.region}:${geoStack.account}:geofence-collection/${EXISTING_COLLECTION_NAME}`,
    ],
    detail: {
      EventType: ["ENTER"],
    },
 ...
 */
geofenceRule.addTarget(
  new LambdaFunction(backend.geofenceHandler.resources.lambda),
);
